...
export default function Page() {
  // ✅ Google Sheets'ten gelen kullanıcı profili
  ...
  // ✅ me çek (cookie ile session kontrolü de buradan dolaylı doğrulanır)
  useEffect(() => {
    (async () => {
      try {
        const r = await fetch("/api/sheets?action=me", { cache: "no-store" });
        const data = await r.json();
        if (!data?.ok) throw new Error(data?.error || "ME_FAILED");
        setMe(data.me || null);
      } catch {
        // me gelmezse login'e at
  ...
  async function onSubmit() {
    ...
    if (!me || me.permission !== "admin") {
      setErr("Bu işlem sadece yönetici (admin) yetkisi ile yapılabilir.");
      return;
    }
    ...
    try {
      ...
      const r = await fetch("/api/sheets", {
        ...
        body: JSON.stringify({
          ...
          summary: sum,
          hotel_name: me.hotel_name || "Opsstay Hotel",
        ...
        <div className="mx-auto max-w-3xl px-6 py-10">
          <div className="rounded-2xl border border-slate-700/40 bg-slate-900/35 backdrop-blur p-6">
            ...
            <p className="mt-2 text-sm text-slate-300">
              Bu sayfa sadece <b>yönetici (admin)</b> içindir.
            </p>
          </div>
        </div>
      </div>
